...
class TaskService:
    ...
    async def edit_task(self, task_id: str, edit_task_data: EditTask, user_id: str) -> Optional[TaskR]:
        ...
        if not task:
            return TaskHandler.handle_task_not_found()
        task.title = edit_task_data.title if edit_task_data.title is not "" else task.title
        task.date = edit_task_data.date if edit_task_data.date is not "" else task.date
        task.description = edit_task_data.description if edit_task_data.description is not "" else task.description
        task.status = edit_task_data.status if edit_task_data.status is not "" else task.status
        self.db.commit()
