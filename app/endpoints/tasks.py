...
async def create_task(add_task: AddTask, current_user: User = Depends(get_current_user), task_service: TaskService = Depends(get_task_service)):
    task = await task_service.create_task(current_user.id, add_task)
    return task


@router.get("/{task_id}/", response_model=TaskR)
async def get_task_by_id(task_id, current_user: User = Depends(get_current_user), task_service: TaskService = Depends(get_task_service)):
...
async def delete_task(task_id: str, current_user: User = Depends(get_current_user), task_service: TaskService = Depends(get_task_service)):
    return await task_service.delete_task(task_id, current_user.id)
        


@router.get("/search", response_model=list[TaskR])
async def get_todos_by_status_and_title(status: Optional[str] = None, title: Optional[str] = None, current_user: User = Depends(get_current_user), task_service: TaskService = Depends(get_task_service)):
    tasks = await task_service.get_tasks_by_status_and_title(current_user.id, status, title)
    return tasks
